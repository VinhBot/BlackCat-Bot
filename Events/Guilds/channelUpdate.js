...
module.exports = {
    ...
    return database.findOne({ GuildId: oldChannel.guild.id, GuildName: oldChannel.guild.name }).then(async(getData) => {
      if(!getData) return;
      const channels = oldChannel.guild.channels.cache.find((channel) => {
        return channel.id === getData.channelUpdate;
      });
      if(!channels) return;
